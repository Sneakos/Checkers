...
public class TextBasedAI extends TextBasedFour
{
    int choice;
    final int maxDepth = 7;
    int turn;

    public TextBasedAI(int turn)
    {
        choice = 0;
        this.turn = turn;
    }
    ...
    public int[][] getBoard(int col, int player, int[][] board)
    {
        ...
        int row = TextBasedFour.getDepth(col, board);
        game[row][col] = player;
    ...
    public int score(int[][] board, int depth)
    {
        ...
        {
            row = TextBasedFour.getDepth(i, board) + 1;
            if(row <= 5)
                win = TextBasedFour.check(row, i, board, board[row][i]);
        }
        if(win == turn) //player
            return 10-depth;
        else if(win == 0) 
